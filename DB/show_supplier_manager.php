...
session_start();
    ...
    echo "</form><br><br>";
    echo "<h1>供應商清單<h1>";
    $sql="select * From 供應商 order by 供應商id";
    $result=$db->query($sql);
    echo "<form action=index1.php method=POST>";   
    echo "<table class='table table-striped'>";
    echo "<tr><td></td><td>供應商id</td><td>供應商名稱</td><td>電話</td><td>地址</td></tr>";        
             ...
             $start = ($page-1)*$per;//記錄偏移量
               ...
               $sql="select * From 供應商 order by 供應商id LIMIT $per  OFFSET $start";
            $result = $db->query($sql);

    foreach($result->fetchAll() as $row)
    {
       
       echo "<tr><td><input type=radio name=supplier_id value=$row[供應商id]></td><td>$row[供應商id]</td><td>$row[供應商名稱]</td><td>$row[電話]</td><td>$row[地址]</td></tr>";        
    }
    echo "<tr><td><button type='submit' name='func' value='insert_supplier'class='btn btn-success'>新增供應商</button></td>";
    echo "<td><button type='submit' name='func' value='update_supplier' class='btn btn-warning'>修改</button></td>";
    echo "<td><button type='submit' name='func' value='delete_action_supplier' class='btn btn-danger'>刪除</button></td><td></td><td></td></tr>";
    echo "</table></form>";
    ...
    echo "<form action='show_supplier_manager.php' method='POST'>
